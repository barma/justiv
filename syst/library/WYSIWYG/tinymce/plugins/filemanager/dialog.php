...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_SESSION["subfolder"]) && strpos($_SESSION["subfolder"], '..' . DIRECTORY_SEPARATOR) === false
        && strpos($_SESSION["subfolder"], '.' . DIRECTORY_SEPARATOR) === false && strpos(
            $_SESSION["subfolder"],
            DIRECTORY_SEPARATOR
        ) !== 0
    ...
    while ($cycle) {
        ...
        if ($parent == "") {
            ...
            $parent = dirname($parent) . DIRECTORY_SEPARATOR;
        }
    }
...
//view type
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="tab1">
                        ...
                        <form action="dialog.php" method="post" enctype="multipart/form-data" id="myAwesomeDropzone"
                            ...
                            <input type="hidden" name="added_path" value="<?php echo $upload_dir . $subdir ?>"/>
                        ...
                        <div class="row-fluid">
                            ...
                            <div class="span6 types">
                                ...
                                <input accesskey="f" type="text" class="filter-input" id="filter-input" name="filter"
                                       placeholder="<?php echo strtolower(lang_Text_filter); ?>..." value=""/>

                                <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all"
                                       class="hide"/>
                                <label id="ff-item-type-all" title="<?php echo lang_All; ?>" for="select-type-all"
            ...
            if (!empty($bc)) {
                foreach ($bc as $k => $b) {
                    ...
                    if ($k == count($bc) - 2) {
                        ...
                        <li><span class="divider"><?php echo DIRECTORY_SEPARATOR; ?></span></li>
                    <?php
                    }
                }
            }
    ...
    <?php if ($show_sorting_bar) { ?>
        ...
        <div class="sorter-container <?php echo "list-view" . $view; ?>">
            <div class="file-name"><a class="sorter" href="javascript:void('')"
                                      data-sort="name"><?php echo lang_Filename; ?></a></div>
            <div class="file-date"><a class="sorter" href="javascript:void('')"
                                      data-sort="date"><?php echo lang_Date; ?></a></div>
            <div class="file-size"><a class="sorter" href="javascript:void('')"
                                      data-sort="size"><?php echo lang_Size; ?></a></div>
            <div class='img-dimension'><?php echo lang_Dimension; ?></div>
            <div class='file-extension'><a class="sorter" href="javascript:void('')"
                                           data-sort="extension"><?php echo lang_Type; ?></a></div>
    ...
    foreach ($files as $file) {
        ...
        if ($file == '.' || !is_dir($root . $cur_dir . $file) || ($file == '..' && $subdir == '') || in_array(
                $file,
                $hidden_folders
            )
        ) {
        ...
        <li data-name="<?php echo $file ?>" <?php if ($file == '..') {
            ...
            <figure class="<?php if ($file == "..") {
                echo "back-";
            } ?>directory">
                <a title="<?php echo lang_Open ?>" class="folder-link"
                   href="dialog.php?<?php echo $get_params . $src . "&" . uniqid() ?>">
                    <div class="img-precontainer">
                        <div class="img-container directory"><span></span>
                            <img class="directory-img" src="ico/folder<?php if ($file == "..") {
                                echo "_return";
                            } ?>.png" alt="folder"/>
                        </div>
                    </div>
                ...
                <?php } else { ?>
                    ...
                    <div class="box">
                        <h4><a title="<?php echo lang_Open ?>" class="folder-link" data-file="<?php echo $file ?>"
                               href="dialog.php?<?php echo $get_params . $src . "&" . uniqid(
                                   ) ?>"><?php echo $file ?></a></h4>
                    </div>
                    ...
                    <?php if ($show_folder_size) { ?>
                        <div class="file-size"><?php echo makeSize($size) ?></div><?php } ?>
                    <div class='file-extension'><?php echo lang_Type_dir; ?></div>
                    <figcaption>
                        <a href="javascript:void('');"
                           class="tip-left edit-button <?php if ($rename_folders) {
                               echo "rename-folder";
                           } ?>"
                           title="<?php echo lang_Rename ?>" data-path="<?php echo $root . $cur_dir . $file; ?>"
                           data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
                            <i class="icon-pencil <?php if (!$rename_folders) {
                                echo 'icon-white';
                            } ?>"></i></a>
                        <a href="javascript:void('')"
                           class="tip-left erase-button <?php if ($delete_folders) {
                               echo "delete-folder";
                           } ?>"
                           title="<?php echo lang_Erase ?>" data-confirm="<?php echo lang_Confirm_Folder_del; ?>"
                           data-path="<?php echo $root . $cur_dir . $file; ?>"
                           data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
    ...
    if ($file == '.' || $file == '..' || is_dir($root . $cur_dir . $file) || in_array(
            $file,
            $hidden_files
        ) || !in_array($file_ext, $ext)
    ) {
        continue;
    }
    ...
    if (in_array($file_ext, $ext_img)) {
        ...
        $thumb_path = str_replace(
            ...
            dirname(
                __FILE__
            ) . DIRECTORY_SEPARATOR . "thumbs" . DIRECTORY_SEPARATOR . $subfolder . DIRECTORY_SEPARATOR . $subdir . $file
        );
        //$file_path=dirname( __FILE__ ). DIRECTORY_SEPARATOR.$current_path.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file;
        $file_path = $root . $upload_dir . $subdir . $file;
    ...
    <li class="ff-item-type-<?php echo $class_ext; ?>" data-name="<?php echo $file ?>">
        <figure>
            <a href="javascript:void('');" title="<?php echo lang_Select ?>" class="link"
               data-file="<?php echo $file; ?>" data-type="<?php echo $_GET['type']; ?>"
               data-field_id="<?php echo $_GET['field_id']; ?>" data-function="<?php echo $apply; ?>">
                ...
                <div class="img-container-mini <?php if ($is_img) {
                    echo 'original-thumb'
} ?>">
            ...
            <div class="box">
                <h4><a href="javascript:void('');" title="<?php echo lang_Select ?>" class="link"
                       data-file="<?php echo $file; ?>" data-type="<?php echo $_GET['type']; ?>"
                       data-field_id="<?php echo $_GET['field_id']; ?>" data-function="<?php echo $apply; ?>">
                        <?php $filename = substr($file, 0, '-' . (strlen($file_ext) + 1));
                        echo $filename; ?></a></h4>
            </div>
            ...
            <figcaption>
                <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                    ...
                    <input type="hidden" name="upload_path" value="<?php echo $root . $cur_dir . $file ?>"/>
                    ...
                    <?php } ?>
                    <a href="javascript:void('');"
                       class="tip-left edit-button <?php if ($rename_files) {
                           echo "rename-file";
                       } ?>"
                       title="<?php echo lang_Rename ?>" data-path="<?php echo $root . $cur_dir . $file; ?>"
                       data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
                    ...
                    <a href="javascript:void('');"
                       class="tip-left erase-button <?php if ($delete_files) {
                           echo "delete-file";
                       } ?>"
