...
class JUpload
{
        ...
        if (count($this->files) > 0) {
            if (isset($this->classparams['callbackAfterUploadManagement'])) {
                $this->logDebug(
                    ...
                    'Before call of ' . $this->classparams['callbackAfterUploadManagement']
                );
                $strForFListContent = call_user_func(
    ...
    private function receive_uploaded_files()
    {
        ...
        foreach ($_FILES as $key => $value) {
            ...
            if (isset($this->classparams['allowed_file_extensions']) && is_array(
                    $this->classparams['allowed_file_extensions']
                )
            ) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'HEAD') {
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'GET') {
                ...
                if (isset($_GET['afterupload'])) {
                    ...
                    if (!isset($_SESSION[$this->classparams['var_prefix'] . 'files'])) {
                        $this->abort(
                            'Invalid session (in afterupload, GET, check of $_SESSION): files array is not set'
                        );
                    }
                    $this->files = $_SESSION[$this->classparams['var_prefix'] . 'files'];
                    if (!is_array($this->files)) {
