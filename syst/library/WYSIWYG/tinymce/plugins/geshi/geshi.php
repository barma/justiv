                    ...
                    if ($string_started && $i != $next_comment_regexp_pos) {
                        ...
                        do {
                            ...
                            if ($simple_escape < $next_escape_regexp_pos &&
                                ...
                                if ($next_escape_regexp_pos < $length &&
                                    ...
                                    $string .= $this->hsc(
                                            substr($part, $start, $close_pos - $start + $char_len)
                                        ) . '</span>';
                        ...
                        if ($this->lexic_permissions['STRINGS'] && $hq && $hq[0] == $char &&
                            ...
                            if (!$COMMENT_MATCHED) {
                                ...
                                if ($i == $next_comment_multi_pos) {
                                    ...
                                    // Short-cut through all the multiline code
                                    $rest_of_comment = $this->hsc(
                                        substr($part, $i + $open_strlen, $close_pos - $i - $open_strlen + $close_strlen)
                                    );
                                    if (($this->lexic_permissions['COMMENTS']['MULTI'] ||
                                ...
                                if ($next_comment_single_pos == $i) {
                                    ...
                                    if ((empty($sc_disallowed_before) || ($i == 0) ||
                                        ...
                                        if ($this->lexic_permissions['COMMENTS'][$comment_key]) {
                                            ...
                                            $test_str = "<span$attributes>" . $this->hsc(
                                                    $this->change_case($comment_mark)
                                                );
                                        } else {
                                        ...
                                        $test_str .= $this->hsc(
                                            substr($part, $i + $com_len, $close_pos - $i - $com_len)
                                        );
                                        if ($this->lexic_permissions['COMMENTS'][$comment_key]) {
                                            $test_str .= "</span>";
                                        }
...
//        $result = substr($result, 1, -1);
    ...
    function handle_regexps_callback($matches)
    {
        ...
        return ' style="' . call_user_func(
            $this->language_data['STYLES']['REGEXPS'][$this->_rx_key],
            $matches[1]
        ) . '"' . $matches[1] . '|>';
    }
...
//        }
        ...
        foreach (array_keys($this->language_data['KEYWORDS']) as $k) {
            if (!isset($this->lexic_permissions['KEYWORDS'][$k]) ||
                ...
                for ($set = 0, $set_length = count(
                    $this->language_data['CACHED_KEYWORD_LISTS'][$k]
                ); $set < $set_length; ++$set) {
...
//                }
        ...
        if ($this->lexic_permissions['SYMBOLS'] && !empty($this->language_data['SYMBOLS'])) {
            ...
            $n_symbols = preg_match_all(
                "/<\|(?:<DOT>|[^>])+>(?:(?!\|>).*?)\|>|<\/a>|(?:" . $this->language_data['SYMBOL_SEARCH'] . ")+/",
                $stuff_to_parse,
                $pot_symbols,
    ...
    function merge_arrays()
    {
        ...
        for ($i = 0; $i < $narrays; $i++) {
            if (!is_array($arrays[$i])) {
                ...
                trigger_error(
                    'Argument #' . ($i + 1) . ' is not an array - trying to merge array with scalar! Returning false!',
                    E_USER_WARNING
                );
                return false;
            }
...
// RYAN JUCKETT: Added this to close the new span I added above.
    ...
    function optimize_regexp_list($list, $regexp_delimiter = '/')
    {
        ...
        $new_entry = $this->_optimize_regexp_list_tokens_to_string($tokens);
        if (GESHI_MAX_PCRE_SUBPATTERNS && $num_subpatterns + substr_count(
                $new_entry,
                '(?:'
