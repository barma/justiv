...
if (!empty($user)) {
    ...
    if (@mysql_num_rows($sql) > 0) {
        if (!mysql_query("update `user` set `ban` = '0' where `user_id` = '$user'")) {
            replaceerror('ошибка!', 1);
        } //else replaceerror('бан снят',2);
        else {
            replaceerror("<div id='user_$user'><a href='#_s' onclick='banned($user);'>забанить</a></div>", 2);
        }
    } else {
